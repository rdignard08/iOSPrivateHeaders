...
@interface CUIRenditionKey : NSObject <NSCopying, NSCoding> {
 ...
 - (id) initWithThemeElement:(long long)a themePart:(long long)b themeSize:(long long)c themeDirection:(long long)d themeValue:(long long)e themeDimension1:(long long)f themeDimension2:(long long)g themeState:(long long)h themePresentationState:(long long)i themeLayer:(long long)j themeScale:(long long)k themeIdentifier:(long long)l ;
