...
@property (nonatomic, assign, readwrite) NSNumber* contactDelegate;
 ...
 - (BOOL) stepWithTime:(double)a velocityIterations:(unsigned long long)b positionIterations:(unsigned long long)c ;
 - (id) bodies;
 - (id) init;
 ...
 - (void) enumerateBodiesInRect:({CGRect={CGPoint=dd}{CGSize=dd}})a usingBlock:(@?)b ;
 - (void) enumerateBodiesAtPoint:({CGPoint=dd})a usingBlock:(@?)b ;
 - (void) enumerateBodiesAlongRayStart:({CGPoint=dd})a end:({CGPoint=dd})b usingBlock:(@?)c ;
 - ({CGVector=dd}) gravity;
 - (double) speed;
 - (id) contactDelegate;
 - (void) __init__;
 ...
 - (id) bodyAlongRayStart:({CGPoint=dd})a end:({CGPoint=dd})b ;
 ...
 - (id) sampleFields:({CGRect={CGPoint=dd}{CGSize=dd}})a categories:(unsigned int)b dataSize:({CGSize=dd})c ;
 - ({CGVector=dd}) sampleFields:({CGPoint=dd})a ;
 - ({CGVector=dd}) evalForce:(unsigned int)a point:({CGPoint=dd})b ;
 - ({b2Vec2=ff}) _gravity;
 - (BOOL) _doSleep;
 - (id) _bodies;
 - (void) debugDraw:(float)a matrix:((_GLKMatrix4={?=ffffffffffffffff}[16f]))b showsPhysics:(BOOL)c showsOutlineInterior:(BOOL)d showsFields:(BOOL)e ;
