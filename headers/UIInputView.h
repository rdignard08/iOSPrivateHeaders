...
@interface UIInputView : UIView <UISplittableInputView> {
 ...
 - (void) layoutSplitSubviewsWithLeftContentSize:({CGSize=dd})a rightContentSize:({CGSize=dd})b ;
 - (void) updateSplitSubviewContraintsWithLeftContentSize:({CGSize=dd})a rightContentSize:({CGSize=dd})b ;
 - (void) willBeginSplitTransition;
 - (void) didEndSplitTransition;
 - (void) _setLeftOffset:(double)a gapWidth:(double)b ;
 - (BOOL) _isToolbars;
 - (void) _setProgress:(double)a boundedBy:(double)b ;
 - (void) setContentRatio:(double)a ;
 - (id) leftContentView;
 - (id) rightContentView;
 - (id) initWithFrame:({CGRect={CGPoint=dd}{CGSize=dd}})a inputViewStyle:(long long)b ;
