...
@property (nonatomic, assign, readonly) NSNumber* paddedInsets;
 ...
 - (id) _copyForDirection:(long long)apositionFactor:(double)bsizeFactor:(double)cscale:(double)d;
 - (void) applyOffset:({CGPoint=dd})a;
 - (void) makeIntegralWithScale:(double)a;
 - (void) adjustToTopWithInsets:({UIEdgeInsets=dddd})a;
 - (id) copyForFlickDirection:(long long)ascale:(double)b;
 - (id) copyForPopupDirection:(long long)ascale:(double)b;
 - (unsigned long long) adjustForTranslucentGapsInFrameWithSize_10Key:({CGSize=dd})acenterX:(double)bisInBottomRow:(BOOL)c;
 ...
 - (id) iPadVariantGeometries:(unsigned long long)arowLimit:(long long)b;
 - (id) similarShape;
 - ({UIEdgeInsets=dddd}) paddedInsets;
 - (unsigned long long) adjustForTranslucentGapsWithSize:({CGSize=dd})ainFrame:({CGRect={CGPoint=dd}{CGSize=dd}})b;
