...
@interface UIRefreshControl : UIControl {
 ...
 - (double) _visibleHeightForContentOffset:({CGPoint=dd})a origin:({CGPoint=dd})b ;
 - (void) _setVisibleHeight:(double)a ;
 - (double) _stiffnessForVelocity:(double)a ;
 - (void) _setRefreshControlState:(int)a notify:(BOOL)b ;
 - (BOOL) _canTransitionFromState:(int)a toState:(int)b ;
 - (BOOL) isRefreshing;
 - (double) _snappingHeight;
